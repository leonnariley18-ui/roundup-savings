...
create index if not exists cards_user_idx on cards (user_id) where active;
...
create table if not exists card_choice_categories (
  id             uuid primary key default gen_random_uuid(),
  user_id        uuid not null references auth.users(id) on delete cascade,
  card_id        uuid not null references cards(id) on delete cascade,
...
create index if not exists card_choice_card_idx on card_choice_categories (card_id, effective_from desc);
...
create index if not exists card_decisions_user_idx on card_decisions (user_id, decided_at desc);
...
create table if not exists statement_closes (
  id        uuid primary key default gen_random_uuid(),
  user_id   uuid not null references auth.users(id) on delete cascade,
  card_id   uuid not null references cards(id) on delete cascade,
...
create index if not exists statement_closes_card_idx on statement_closes (card_id, closed_on desc);
...
create table if not exists paybacks (
  ...
  card_id             uuid references cards(id) on delete set null,  -- NULL = off-card
...
create index if not exists paybacks_user_idx on paybacks (user_id, status);

create table if not exists payback_payments (
  id         uuid primary key default gen_random_uuid(),
  user_id    uuid not null references auth.users(id) on delete cascade,
  payback_id uuid not null references paybacks(id) on delete cascade,
  amount     numeric(12,2) not null check (amount > 0),
  paid_at    date not null default current_date
);
create index if not exists payback_payments_pb_idx on payback_payments (payback_id);
...
create index if not exists bill_instances_date_idx on bill_instances (user_id, due_date);
...
create index if not exists notes_date_idx on notes (user_id, on_date);
...
create table if not exists roundup_runs (
  id      uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
...
alter table bills
  ...
  foreign key (links_to_debt_id) references debts(id) on delete set null;
...
create index if not exists events_user_idx on events (user_id, occurred_at desc);
