...
--
--   select public.seed_ledger('00000000-0000-0000-0000-000000000000');
--
...
create or replace function public.seed_ledger(uid uuid)
returns text
language plpgsql
security invoker
as $$
...
begin
  if exists (select 1 from cards where user_id = uid) then
  ...
  insert into cards (user_id, name, last4, close_day, due_day, credit_limit,
  ...
  insert into card_rewards (user_id, card_id, category, value, unit, label_note) values
  ...
  insert into card_rewards (user_id, card_id, category, value, unit, counts_toward_cap)
  ...
  insert into card_choice_categories (user_id, card_id, category, effective_from)
  ...
  insert into debts (user_id, name, principal, apr, minimum_payment, actual_payment,
